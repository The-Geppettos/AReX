...
export abstract class Table<T extends Object = {}> {
  ...
  async insert(instance: T | T[]): Promise<T | null | T[]> {
    ...
    const result = await this.coreDb.query<T>(query, values);
  ...
  async getById(id: string) {
    const result = await this.coreDb.query<T>(
  ...
  async count(query: Partial<T> = {}): Promise<number> {
    ...
    const result = await this.coreDb.query<{ count: string }>(
  ...
  async update(query: Partial<T>, instance: Partial<T>): Promise<T[]> {
    ...
    const queryText = `UPDATE ${this.tableName}
                       ...
                       WHERE ${whereClause}
                       RETURNING *;`;
    const result = await this.coreDb.query<T>(queryText, values);
